  ...
  return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center' }}>
      ...
      <div className="animate-fade-in" style={{ textAlign: 'center', marginBottom: '40px' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', marginTop: '8px' }}>RuralCareX Medical Staff</p>
      </div>
      ...
      <div className="glass-card animate-slide-up delay-1" style={{ padding: '32px 24px' }}>
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            <input 
              type="text"
              placeholder="e.g. drsharma@test.com"
              value={identifier}
          ...
          <button type="submit" className="btn-primary" style={{ marginTop: '8px', background: 'var(--secondary)' }} disabled={loading}>
            {loading ? 'Logging in...' : 'Login securely'} <ArrowRight size={18} />
          </button>
        </form>
      </div>
      
      <div style={{ textAlign: 'center', marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <Link to="/doctor-register" style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', textDecoration: 'none' }}>
          Don't have an account? <span style={{ color: 'var(--primary)' }}>Register here</span>
        </Link>
        <Link to="/login" style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', textDecoration: 'none' }}>
          Are you a patient? <span style={{ color: 'var(--primary)' }}>Login here</span>
        </Link>
      </div>
